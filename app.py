...
# Add this test route near the top of your routes
...
try:
    ...
    if not os.path.exists(firebase_cred_path):
        raise ValueError(f"Firebase credentials file not found at {firebase_cred_path}")
    ...
    if not firebase_admin._apps:
        firebase_admin.initialize_app(cred, {
            'storageBucket': 'quickbyte-f4aba.appspot.com'
        })
...
def login():
    try:
        ...
        try:
            user_ref = db.collection("users").document(email)
            user = user_ref.get(timeout=30)  # Add timeout
            
            if not user.exists:
                return jsonify({"error": "Invalid credentials"}), 401
            
            user_data = user.to_dict()
            if not pbkdf2_sha256.verify(password, user_data.get("password", "")):
                return jsonify({"error": "Invalid credentials"}), 401

            # Generate token with fixed expiration
            token_data = {
                "email": email,
